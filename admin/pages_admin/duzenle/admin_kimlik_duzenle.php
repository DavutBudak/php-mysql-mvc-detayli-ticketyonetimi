<?php
    $id = intval($_GET["id"]);
    $uyefoto = $db->prepare("SELECT * FROM uye_kimlik_dosyalari WHERE id = ?");
...
<div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Kimlik Düzenle</h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-home"></i> Anasayfa</a></li> 
        <li><a href="#"><i class="fa fa-home"></i> Menü</a></li> 
        <li><a href="#"><i class="fa fa-home"></i>Kimlik Düzenle</a></li> 
        <li><a href="#"><i class="fa fa-home"></i> Kimlik Düzenleme Sayfası</a></li> 
      </ol>
    </section>
...
if (isset($_FILES['uye_kimlik'])) {
    ...
    for ($i = 0; $i < $resim_sayisi; $i++) {
        ...
        if ($resimBoyutu > (1024 * 1024 * 2)) {
            ...
            $yuklenemeyenler[] = $_FILES['uye_kimlik']['name'][$i] . " - BOYUT";
        } else {
            $tip = $_FILES['uye_kimlik']['type'][$i]; //resim tipini öğrendik.
            $resimAdi = $_FILES['uye_kimlik']['name'][$i]; //resmin adını öğrendik.
            
            if ($tip == 'image/jpeg' || $tip == 'image/jpg' || $tip == 'image/png') { //uzantısnın kontrolünü sağladık. sadece .jpg ve .png yükleyebilmesi için.
                if (move_uploaded_file($_FILES["uye_kimlik"]["tmp_name"][$i], $klasor . "/" .$randomubunaal=$rand. $_FILES['uye_kimlik']['name'][$i])) {
                    ...
                    $uye_kimlik = $randomubunaal;

                    $resim_tarih = date("Y-m-d H:i:s");
              
                    $resimekle = $db->prepare("UPDATE uye_kimlik_dosyalari SET uye_kimlik = ? , resim_tarih = ? WHERE id = ?");
                    $resimekle->execute(array($uye_kimlik,$resim_tarih, $id));
...
?>
            ...
            <div class="col-md-12 form-group border">
                      <table>
                        <tr>
                          <td>Dosya Seçiniz:</td>
                          <td><input type="file" name="uye_kimlik[]"></td>
